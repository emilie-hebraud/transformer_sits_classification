...
def eval(yaml_path=None,model_path=None,test_metrics_path=None,conf_mat_path=None):
    results_folder = "/home/emilie/Documents/IODAA/classification_site/tutorials/results"

    with open(os.path.join(results_folder, yaml_path), 'r') as file:
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, cfg["batch_size"], shuffle=True, collate_fn=padding.pad_collate
    )
    ...
    checkpoint = torch.load(os.path.join(results_folder, model_path), map_location=device)
    ...
    with open(os.path.join(results_folder,test_metrics_path), 'w') as file:
        yaml.dump(metrics, file)

    pkl.dump(cm, open(os.path.join(results_folder, conf_mat_path), 'wb'))
